...
- Judge recommender changes with `python tools/recommender_report.py` (coverage, top-1 tie rate, simulated-session shape, repeats) rather than by argument. It drives real sessions through `PlayerEngine.advance()`, so it exercises the queue top-up, the artist cap and the work-key cooldown the way playback does. Baseline on the reference library: 28% top-1 ties, ~0.77 similarity between consecutive picks, ~3 genre labels per 60-pick session, 0 sessions replaying a song.
