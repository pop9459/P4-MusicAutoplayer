...
A local, offline music recommender/player. It scans folders of audio files (`.mp3 .m4a .flac .wav .ogg .aac`) into a central JSON library, recommends similar next tracks by comparing their metadata pairwise (`track_similarity`, not cosine over a vector space), and plays them through a curses-based TUI backed by `mpv`. Tracked folders are never written to — all catalog/library data lives centrally (`data/library.json`), never inside a scanned music folder.
...
```bash
python -m src.cli add-folder --path /path/to/music     # recursive scan; rescans (refreshes metadata) if the path is already tracked
...
- `src/library.py` owns tracked folders and the merged catalog in one central `Library` (`folders: list[LibraryFolder]`, `catalog: Catalog`), persisted as one `data/library.json`. `add_folder` scans only the new folder, merges its tracks into the existing catalog, and rebuilds the catalog's feature space over *all* tracks (not just the new folder) so the feature-space invariant below still holds; adding an already-tracked (resolved-path) folder rescans it via `rescan_folder` rather than no-op'ing, and reports `was_added=False` so callers can word the result correctly. `remove_folder` is the CLI-only counterpart. `migrate_settings_to_library` is a one-time, non-destructive migration from pre-library `settings.json`/`catalog_path` setups — it never deletes the old catalog file. `start_add_folder_task` runs a scan on a background `threading.Thread` (never touching curses/mpv) so the TUI can poll `ScanTask.progress()` each render tick instead of freezing during a large scan.
- `src/predictor.py` is the pure recommender core: ranks candidates by per-pair `track_similarity` (not cosine over a vector space), filters ineligible/current/excluded tracks, applies the artist-repeat cap and work-key cooldown, selects from top-k with optional weighted randomness, and generates no-repeat session queues. `generate_queue_steps` blends each step's score with similarity to the *seed* track at `SEED_ANCHOR_WEIGHT = 0.3` (`rank_candidates_by_features(anchor=...)`): chaining purely off the last pick lets small drifts compound, so a long queue can end up sounding nothing like where it started. Note this anchors **queue generation**, not the player's incremental top-up (`PlayerEngine.top_up_queue_steps`), which chains off the last pick — that is the distinction issue #10 measured.
...
- `src/queue_panel.py` — right column: upcoming queue display. Display-only: no key focuses or scrolls it, and `update_queue` is called on every reveal tick, so any scroll state would not survive anyway.
- `src/player_bar.py` — bottom bar: current track, play state, progress bar, status messages. Also owns the two shared display helpers `format_time` and `track_line`.
- `src/settings_panel.py` — full-screen settings editor (`s`), editing a copy of `Settings`: `top_k`, `randomness`, `queue_length`, `normalize_volume`, `library_path`. `to_settings()` clears the legacy migration fields, so saving from the TUI makes `settings.json` preferences-only.
- `src/player_ui_v2.py` — integrates all four panels into one curses event loop with navigation and playback control. Startup: load settings/library → init `FolderPanel` → load the selected folder's songs into `SongsPanel` → init `PlayerEngine` with first song → render and loop. The middle column's header shows the selected folder's name/path/track-count above its song list. Pressing `b` in the folder column starts (or stops) background tempo analysis via `start_bpm_task`, polled by `_poll_bpm_task` each tick: detected tempi are applied to the live catalog on the UI thread, the updated tracks' cached `TrackFeatures` are dropped so they re-derive with the new tempo, and the library is saved every `_BPM_SAVE_EVERY` results plus once on exit (the worker is a daemon thread, so quitting would otherwise discard everything since the last save). Adding a folder is refused while analysis runs, since `add_folder` replaces `self.library` and would strand in-flight results. Pressing `a` in the folder column prompts (blocking `curses.echo()`/`getstr()`, the same pattern used for settings text edits — a path pasted via drag-and-drop lands in the same prompt) for a folder path and runs the scan on a background thread (`library.start_add_folder_task`), polled each 200ms loop tick so scan progress ("Scanning: N/Total") renders without freezing playback controls. Pressing `u` on a selected tracked folder (not the virtual "All Tracks" entry) re-scans it without a path prompt, by calling `library.start_add_folder_task` with the folder's already-known path — the exact same task/poll machinery `a` uses, since `add_folder` itself rescans (via `rescan_folder`: refreshes metadata such as `duration` for files still on disk, drops tracks whose files were removed, picks up new ones, preserves each surviving track's `enabled` flag) rather than no-op'ing when given a path that's already tracked. Both keys share one `_scan_task`/`_poll_scan_task` pair, so they're mutually exclusive with each other and with a BPM run for the same reason `add_folder` and analysis already exclude each other — either replaces `self.library` wholesale.
...
- Preserve catalog serialization compatibility: `CATALOG_VERSION` is `3`, `LIBRARY_VERSION` is `2`, settings `version` is `1`; `save_catalog`/`save_library` emit indented, ASCII JSON with a final newline. A v1 file still loads — its per-track `feature_vector` is ignored and dropped on the next save. A pre-v3 catalog has no `duration`; it defaults to `None` and is backfilled on the next rescan, same as a missing BPM tag.
...
- Judge recommender changes with the measurement tools rather than by argument. There are three, and they answer different questions — reaching for the wrong one is why "is it actually good?" used to be unanswerable:
  ...
  The two that simulate sessions (`recommender_report.py`, `ab_listen.py`) drive them through the shared `tools/_session.py` `play_session` helper, so both exercise the queue top-up, the artist cap and the work-key cooldown the way playback does. Keep it that way — divergent copies would quietly stop measuring the same thing. `eval_holdout.py` has no session to drive: it scores album-mate retrieval one seed at a time.
